...
class SimpleSAGEConv(nn.Module):
    """
    Simplified SAGE-style graph convolution using pure PyTorch.
    Implements message passing through the 64-gate quantum field.
    """
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        # Scatter add neighbor features to each target node
        neighbor_sum = torch.zeros_like(x)
        neighbor_count = torch.zeros(num_nodes, 1, device=x.device)
        
        for src, tgt in zip(source_nodes.tolist(), target_nodes.tolist()):
            neighbor_sum[tgt] += x[src]
        ...
        out_neigh = self.lin_neigh(neighbor_mean)
...
class QuantumHDNet(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_layers):
            self.convs.append(SimpleSAGEConv(hidden_dim, hidden_dim))
    ...
    def forward(
        ...
        Returns:
            ...
            awareness_vectors: dict of pooled awareness vectors
            h_observed: [64, hidden_dim] observed node embeddings
        """
...
class QuantumFieldLoss(nn.Module):
    ...
    def forward(
        ...
        awareness_sum = torch.stack(list(awareness.values())).sum()
